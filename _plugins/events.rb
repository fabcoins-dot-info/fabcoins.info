  ...
  class EventPageGenerator < Generator
	...
	date = data['date'].to_s.split('-')
        ...
        if data.has_key?('address')
          begin
            geoloc = JSON.parse(open("http://maps.googleapis.com/maps/api/geocode/json?address=" + CGI::escape(data['address'] + ', ' + data['city'] + ', ' + data['country']) + "&sensor=false","User-Agent"=>"Ruby/#{RUBY_VERSION}").read)
            if geoloc['status'] == 'OK'
