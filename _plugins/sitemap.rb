  ...
  class SitemapGenerator < Generator
      ...
      File.open(File.join(site.dest, 'sitemap.xml'), 'w+') do |sitemap|
        ...
        locs['en']['url'].each do |id,value|
          locs.each do |lang,value|
            ...
            sitemap.puts '<url>'
            sitemap.puts '  <loc>http://fabcoins.info/'+lang+'/'+CGI::escape(locs[lang]['url'][id])+'</loc>'
            locs.each do |altlang,value|
              ...
              sitemap.puts '    href="http://fabcoins.info/'+altlang+'/'+CGI::escape(locs[altlang]['url'][id])+'" />'
            end
            sitemap.puts '</url>'
          end
        end
	#Add static non-translated pages
        Dir.glob('en/**/*.{md,html}').concat(Dir.glob('*.{md,html}')).each do |file|
          ...
          sitemap.puts '<url>'
          sitemap.puts '  <loc>http://fabcoins.info/'+file.gsub('.html','').gsub('.md','')+'</loc>'
          sitemap.puts '</url>'
        end
        #Add alerts pages
        Dir.foreach('_alerts') do |file|
          ...
          sitemap.puts '<url>'
          sitemap.puts '  <loc>http://fabcoins.info/en/alert/'+file.gsub('.html','')+'</loc>'
          sitemap.puts '</url>'
        end
        #Add releases pages
        Dir.foreach('_releases') do |file|
          ...
          file = file.join('-')
          sitemap.puts '<url>'
          sitemap.puts '  <loc>http://fabcoins.info/en/release/'+file.gsub('.md','').gsub('.html','')+'</loc>'
          sitemap.puts '</url>'
        end
        #Close sitemap
        sitemap.puts '</urlset>'
