...
{% endcomment %}
{% assign filename="_data/devdocs/en/fabcoin-core/rpcs/rpcs/gettxoutsetinfo.md" %}
...
- n: "→<br>`bytes_serialized`"
  t: "number (int)"
  p: "Required<br>(exactly 1)"
  d: "The size of the serialized UTXO set in bytes; not counting overhead, this is the size of the `chainstate` directory in the Fabcoin Core configuration directory"
...
- n: "→<br>`total_amount`"
  t: "number (fabcoins)"
  p: "Required<br>(exactly 1)"
  d: "The total number of fabcoins in the UTXO set"
...
{% highlight bash %}
fabcoin-cli -testnet gettxoutsetinfo
{% endhighlight %}
