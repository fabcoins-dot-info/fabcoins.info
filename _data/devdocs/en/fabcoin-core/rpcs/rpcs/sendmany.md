...
{% endcomment %}
{% assign filename="_data/devdocs/en/fabcoin-core/rpcs/rpcs/sendmany.md" %}
...
- n: "From Account"
  ...
  d: "*Deprecated: will be removed in a later version of Fabcoin Core*<br><br>The name of the account from which the fabcoins should be spent.  Use an empty string (\"\") for the default account. Fabcoin Core will ensure the account has sufficient fabcoins to pay the total amount in the *outputs* field described below (but the transaction fee paid is not included in the calculation, so an account can spend a total of its balance plus the transaction fee)"
...
- n: "→<br>Address/Amount"
  t: "string (base58) : number (fabcoins)"
  p: "Required<br>(1 or more)"
  d: "A key/value pair with a base58check-encoded string containing the P2PKH or P2SH address to pay as the key, and an amount of fabcoins to pay as the value"
...
From the account *test1*, send 0.1 fabcoins to the first address and 0.2
fabcoins to the second address, with a comment of "Example Transaction".
