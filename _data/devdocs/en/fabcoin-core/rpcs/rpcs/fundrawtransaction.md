...
{% endcomment %}
{% assign filename="_data/devdocs/en/fabcoin-core/rpcs/rpcs/fundrawtransaction.md" %}
...
- n: "Options"
  ...
  d: "*Added in Fabcoin Core 0.13.0*<br><br>Additional options"

- n: "→ <br>`changeAddress`"
  t: "string"
  p: "Optional<br>(0 or 1)"
  d: "The fabcoin address to receive the change. If not set, the address is chosen from address pool"
...
- n: "→ <br>`reserveChangeKey`"
  t: "bool"
  p: "Optional<br>(0 or 1)"
  d: "*Added in Fabcoin Core 0.14.0*<br><br>Reserves the change output key from the keypool. The default is `true`. Before 0.14.0, the used keypool key was never marked as change-address key and directly returned to the keypool (leading to address reuse)."  
  
- n: "→ <br>`feeRate`"
  t: "numeric (fabcoins)"
  p: "Optional<br>(0 or 1)"
  d: "The specific feerate  you are willing to pay(FAB per KB). If not set, the wallet determines the fee"
...
- n: "→ <br>fee"
  t: "numeric (fabcoins)"
  p: "Required<br>(Exactly 1)"
  d: "Fee in FAB the resulting transaction pays"
...
{% highlight bash %}
fabcoin-cli -testnet fundrawtransaction 01000000011da9283b4ddf8d\
