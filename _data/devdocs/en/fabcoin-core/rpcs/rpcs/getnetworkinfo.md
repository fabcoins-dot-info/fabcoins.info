...
{% endcomment %}
{% assign filename="_data/devdocs/en/fabcoin-core/rpcs/rpcs/getnetworkinfo.md" %}
...
- n: "→<br>`localrelay`"
  t: "bool"
  p: "Required<br>(exactly 1)"
  d: "*Added in Fabcoin Core 0.13.0*<br><br>The services supported by this node as advertised in its `version` message"
...
- n: "→ → →<br>`limited`"
  t: "bool"
  p: "Required<br>(exactly 1)"
  d: "Set to `true` if only connections to this network are allowed according to the `-onlynet` Fabcoin Core command-line/configuration-file parameter.  Otherwise set to `false`"
...
- n: "→ → →<br>`proxy_randomize_credentials`"
  t: "bool"
  p: "Required<br>(exactly 1)"
  d: "*Added in Fabcoin Core 0.11.0*<br><br>Set to `true` if randomized credentials are set for this proxy. Otherwise set to `false`"
...
- n: "→<br>`warnings`"
  t: "string"
  p: "Required<br>(exactly 1)"
  d: "*Added in Fabcoin Core 0.11.0*<br><br>A plain-text description of any network warnings. If there are no warnings, an empty string will be returned. "
