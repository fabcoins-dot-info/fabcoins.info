...
WORKDIR=`mktemp -d`
LIVEDIR=`mktemp -d`
SITEDIR='/fabcoins.info/txpreview'
DESTDIR='/var/www/txpreview'
FABCOINORG_BUILD_TYPE='preview'
...
for la in "${LANGS[@]}"
do
	checksum=`sha256sum $SITEDIR/tx/translations/fabcoinorg.fabcoinorg/$la.yml`
	checksum=(${checksum//" "/ })
...
cd $WORKDIR
linecounten=`cat $SITEDIR/tx/translations/fabcoinorg.fabcoinorg/en.yml | grep -o '^ \+[a-z0-9]\+:' | wc -l`
echo '<h1>Needs updating</h1>' > $WORKDIR/diff.html
for la in "${LANGS[@]}"
do
	rsync -a $SITEDIR/tx/translations/fabcoinorg.fabcoinorg/$la.yml $WORKDIR/_translations/$la.yml
