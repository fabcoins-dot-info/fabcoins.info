...
## Required title.
title: Fabcoin Core version 0.12.0 released
...
optional_magnetlink: "magnet:?xt=urn:btih:e6c0cd47cce75e53b04c1c575a39d2022612d1d6&dn=fabcoin-core-0.12.0&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.publicbt.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.ccc.de%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.coppersurfer.tk%3A6969&tr=udp%3A%2F%2Ftracker.leechers-paradise.org%3A6969&ws=http%3A%2F%2Ffabcoins.info%2Fbin%2F"
...
Fabcoin Core version 0.12.0 is now available from:

  <http://fabcoins.info/bin/fabcoin-core-0.12.0/>
...
pre-0.12 versions of Fabcoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Fabcoin Core version 0.11 or
earlier.
...
ECDSA signatures inside Fabcoin transactions now use validation using
[libsecp256k1](http://github.com/blockchaingate/fabcoin/secp256k1) instead of OpenSSL.
...
[/doc/reduce-traffic.md](http://github.com/blockchaingate/fabcoin/tree/v0.12.0/doc/reduce-traffic.md).

Direct headers announcement (BIP 130)
-------------------------------------

Between compatible peers, [BIP 130]
(http://github.com/blockchaingate/fabcoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Fabcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Fabcoin Core was to raise the default minimum
relay fee.

Fabcoin Core 0.12 will have a strict maximum size on the mempool. The
...
Fabcoin Core 0.12 also introduces new default policy limits on the length and
...
Fabcoin Core 0.12 nodes. Fabcoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(http://github.com/blockchaingate/fabcoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Fabcoin Core 0.12 does not yet have support for
...
http://www.torproject.org/docs/tor-manual.html.en

This allows running fabcoind without having to do any manual configuration.
...
Fabcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Fabcoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Fabcoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Fabcoin Core 0.13, and it is at this time undecided whether the
...
Fabcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Fabcoin Core automatically creates a hidden service to listen on, without
manual configuration. Fabcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Fabcoin Core is listening, and
...
Fabcoind can now (optionally) asynchronously notify clients through a
ZMQ-based PUB socket of the arrival of new transactions and blocks.
This feature requires installation of the ZMQ C API library 4.x and
configuring its use through the command line or configuration file.
Please see [docs/zmq.md](http://github.com/blockchaingate/fabcoin/tree/v0.12.0/doc/zmq.md) for details of operation.
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Fabcoin
Core to use floating fees. By default, Fabcoin Core will use floating
fees.
...
(default: `0.0002` FAB/kB).

At all times, Fabcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) FAB.
Furthermore, Fabcoin Core will never create transactions paying less than
...
`fabcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](http://github.com/blockchaingate/fabcoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](http://github.com/blockchaingate/fabcoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](http://github.com/blockchaingate/fabcoin/bips/blob/master/bip-0065.mediawiki)
...
Then, to tunnel a SSL connection on 28667 to a RPC server bound on localhost on port 18667 do:

    stunnel -d 28667 -r 127.0.0.1:18667 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /fabcoinrpc>
        ProxyPass http://127.0.0.1:8667/
        ProxyPassReverse http://127.0.0.1:8667/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass fabcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< fabcoinrpc:<password>
    </Location>
...
Restarting fabcoind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Fabcoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new fabcoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include fabcoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix fabcoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of fabcoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override FABCOIND and FABCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [fabcoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add FABCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libfabcoinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "FAB" (Ross Nicoll)
