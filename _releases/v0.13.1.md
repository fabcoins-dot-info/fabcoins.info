...
## Required title.
title: Fabcoin Core version 0.13.1 released
...
optional_magnetlink: "magnet:?xt=urn:btih:dbe48c446b1113890644bbef03e361269f69c49a&dn=fabcoin-core-0.13.1&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.publicbt.com%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.ccc.de%3A80%2Fannounce&tr=udp%3A%2F%2Ftracker.coppersurfer.tk%3A6969&tr=udp%3A%2F%2Ftracker.leechers-paradise.org%3A6969&ws=http%3A%2F%2Ffabcoins.info%2Fbin%2F"
...
Fabcoin Core version 0.13.1 is now available from:

  <http://fabcoins.info/bin/fabcoin-core-0.13.1/>
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Fabcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Fabcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Fabcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of fabcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of fabcoins being spent is exactly
...
- **Increased security for multisig:** Fabcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Fabcoin's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Fabcoin paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Fabcoin's current ledger (called the UTXO set),
  ...
  note that Fabcoin Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Fabcoin users to individually opt-in to almost any change in the Fabcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Fabcoin Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Fabcoin community members are studying several other improvements
...
[Segwit FAQ]: http://fabcoincore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: http://fabcoincore.org/en/segwit_wallet_dev/
[BIP141]: http://github.com/blockchaingate/fabcoin/bips/blob/master/bip-0141.mediawiki
[BIP143]: http://github.com/blockchaingate/fabcoin/bips/blob/master/bip-0143.mediawiki
[BIP144]: http://github.com/blockchaingate/fabcoin/bips/blob/master/bip-0144.mediawiki
[BIP145]: http://github.com/blockchaingate/fabcoin/bips/blob/master/bip-0145.mediawiki
[versionbits FAQ]: http://fabcoincore.org/en/2016/06/08/version-bits-miners-faq/
...
Since Fabcoin Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: http://github.com/blockchaingate/fabcoin/bips/blob/master/bip-0147.mediawiki
...
- `fabcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `fabcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in fabcoinconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.fabcoinstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libfabcoinconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_fabcoin by itself (luke-jr)
...
- #8829 `2a8bca4` Add fabcoin-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` fabcoin-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_fabcoinds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name fabcoin.qrc  (fabcoinsSG)
...
- #8817 `bcf3806` update fabcoin-tx to output witness data (jnewbery)
