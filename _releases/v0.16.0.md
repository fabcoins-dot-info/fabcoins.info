...
## Required title.
title: Fabcoin Core version 0.16.0 released
...
optional_magnetlink: "magnet:?xt=urn:btih:6493ae7a15b4d32bb4eca1dfaf6dcc0c143492cb&dn=fabcoin-core-0.16.0&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A80&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337&tr=udp%3A%2F%2Ftracker.coppersurfer.tk%3A6969&tr=udp%3A%2F%2Ftracker.leechers-paradise.org%3A6969&tr=udp%3A%2F%2Fzer0day.ch%3A1337&tr=udp%3A%2F%2Fexplodie.org%3A6969"
...
Fabcoin Core version 0.16.0 is now available from:

  <http://fabcoincore.org/bin/fabcoin-core-0.16.0/>
...
Fabcoin Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
fabcoin data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Fabcoin Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µFAB" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- fabcoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- fabcoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18667.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest fabcoin.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.fabcoin.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for fabcoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to fabcoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #10825 `28485c7` Set regtest JSON-RPC port to 18443 to avoid conflict with testnet 18667 (fametrano)
...
- #12035 `eeb6d52` Change µFAB to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_fabcoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` fabcoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.fabcoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for fabcoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in fabcoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop fabcoind (OmeGak)
...
- #10529 `331352f` Improve fabcoind systemd service file (Flowdalic)
