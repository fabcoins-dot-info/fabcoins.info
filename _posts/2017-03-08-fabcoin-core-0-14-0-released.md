...
title: "Fabcoin Core Version 0.14.0 Released"
permalink: /en/posts/fabcoin-core-version-0-14-0-released.html
date: 2017-03-08
author: |
...
{:.center}
![Fabcoin Core Version 0.14.0](/img/blog/free/fabcoin-core-0140.png)

[Fabcoin Core version 0.14.0 is now available](http://fabcoins.info/en/download).
...
Fabcoin Core has supported automatically pruning the blockchain since 0.11.
...
When Fabcoin Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `fabcoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`fabcoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

`src/fabcoin-cli -named help command="help"`
`src/fabcoin-cli -named getblockhash height=0`
`src/fabcoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934...`
`src/fabcoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true`
...
The Alert System was [disabled and deprecated](http://fabcoins.info/en/alert/2016-11-01-alert-retirement)
in Fabcoin Core 0.12.1 and removed in 0.13.0. The Alert System was retired with
...
- \#8421 `b77bb95` httperver: drop boost dependency (theuni)
...
- \#8722 `bb843ad` fabcoin-cli: More detailed error reporting (laanwj)
...
- \#8810 `14e8f99` tests: Add exception error message for JSONRPCException
  (laanwj)
- \#8830 `ef0801b` test: Add option to run fabcoin-util-test.py manually
  (jnewbery)
- \#8881 `e66cc1d` Add some verbose logging to fabcoin-util-test.py (jnewbery)
...
- \#9023 `774db92` Add logging to fabcoin-util-test.py (jnewbery)
- \#9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- \#9069 `ed64bce` Clean up bctest.py and fabcoin-util-test.py (jnewbery)
...
- \#9691 `fc67cd2` Init ECC context for `test_fabcoin_fuzzy` (gmaxwell)
...
- \#8667 `806b9e7` Clarify witness branches in transaction.h serialization
  (dcousens)
...
- \#9865 `289204f` Change fabcoin address in RPC help message (marijnfs)
...
- \#8813 `fb24d7e` fabcoind: Daemonize using daemon(3) (laanwj)
...
- \#8837 `2108911` Allow fabcoin-tx to parse partial transactions (jnewbery)
- \#9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- \#9265 `31bcc66` fabcoin-cli: Make error message less confusing (laanwj)
...
- \#8883 `d5d4ad8` Add all standard TXO types to fabcoin-tx (jnewbery)
